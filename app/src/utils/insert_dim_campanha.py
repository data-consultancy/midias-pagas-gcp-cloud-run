...
_SQL_PATH = Path(__file__).parent / "sql" / "dim_campanha.sql"
...
def insert_dim_campanha():
    ...
    if _is_full_load():
        ...
        logger.info("Modo carga full: lendo todos os registros para %s...", TABLE_DIM_CAMPANHA)
    else:
        target_date = _resolve_target_date()
        raw = raw.replace("{D_MINUS_1}", target_date)
        logger.info("Modo incremental: data alvo = %s | tabela = %s", target_date, TABLE_DIM_CAMPANHA)
    ...
    query = (
        ...
        .replace("{TABLE_DIM_CAMPANHA}", TABLE_DIM_CAMPANHA)
    )

    client.query(query).result()
    logger.info("MERGE concluído. Tabela: %s", TABLE_DIM_CAMPANHA)
